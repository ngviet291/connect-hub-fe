...
const saveAuth = (user: AuthResponse) => {
  store.dispatch(setTokens({ accessToken: user.accessToken, refreshToken: user.refreshToken }));
  store.dispatch(setUser({ email: user.user.email, role: user.user.roles[0], fullName: user.user.fullName }));
...
export const authService = {
  ...
  logout: async () => {
    ...
    try {
      const res = await authApi.logout({ accessToken: accessToken!, refreshToken: refreshToken! });
