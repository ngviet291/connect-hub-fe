...
import { setTokens, setUser, logout as logoutAction, setCurrentUser } from '../store/authSlice';
...
const saveAuth = (res: Awaited<ReturnType<typeof authApi.login>>) => {
  store.dispatch(setTokens({ accessToken: res.accessToken, refreshToken: res.refreshToken }));
  store.dispatch(setUser({ email: res.user.email, role: res.user.roles[0], fullName: res.user.fullName }));
...
export const authService = {
  ...
  logout: async () => {
    ...
    try {
      await authApi.logout({ accessToken: accessToken!, refreshToken: refreshToken! });
    } finally {
  ...
  refreshToken: async (data: { refreshToken: string }): Promise<TokenPair> => {
    const res = await publicClient.post<ApiResponse<TokenPair>>(AUTH_ENDPOINTS.REFRESH_TOKEN, data);
